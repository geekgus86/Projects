$(document).ready(function(){
	...
	$("#nombreEmpresaM").attr("disabled", "disabled");
	$("#puestoM").attr("disabled", "disabled");
	$("#funcionesEmpresaM").attr("disabled", "disabled");
	$("#institucionM").attr("disabled", "disabled");
	...
	$('#talentos').svg();
	 ...
	 obtenerEducacion(svg,g1,g2);
	  ...
	  $("#botonEditar").click(function(){
			document.location.href="/admins/perfil";
		});
	  $("#talentos").show();
			     ...
			     buttons: {
			          ...
			          'Cerrar':function(){
			           ...
			           estudioActualCheckM=document.getElementById("estudioActualM");
				         estudioActualCheckM.checked = false;
				         $("#afaml").show();
				         $("#mesFinFM").show();
				         $("#anioFinFM").show();
			  ...
			  hide: "explode",
			     ...
			     buttons: {
			    	 'Cerrar':function(){
				           ...
				           trabajoActualCheckM.checked = false;
				           $("#aexpml").show();
				           $("#mesFinEM").show();
				           $("#anioFinEM").show();
...
function softwareCat(cat){
	var categoria=cat;
	$.post("/admins/json/ObtenerCatSoftware", {categoria:categoria},
			  function(jsonData) {
		       var i=0;
		       var height=0;
...
function obtenerListaSoftware(categoria){
	$.post("/admins/json/ObtenerSoftware", {categoria:categoria},
			  function(jsonData) {
				var i=0;
...
function obtenerEmpleo(svg,g1,g2){ 
	var IdAdmin = Separ[1];
	$.post("/admins/json/ObtenerEmpleo", {IdAdmin:IdAdmin},
			  function(jsonData) {
		        var i=0;
		        for(i=0;i<jsonData.items.length;i++){
...
function obtenerEducacion(svg,g1,g2){
	var IdAdmin = Separ[1];
	$.post("/admins/json/ObtenerEducacion", {IdAdmin:IdAdmin},
			  function(jsonData) {
		        var i=0;
		        for(i=0;i<jsonData.items.length;i++){
...
function obtenerHobbies(){
	var IdAdmin = Separ[1];
	$.post("/admins/json/ObtenerHobbies", {IdAdmin:IdAdmin},
			function(jsonData) {
		var i=0;
    for(i=0;i<jsonData.items.length;i++){
...
function dibujarGraficaTalentos(svg){
	 ...
	 svg.rect(135,40,10,10,1,1,{id:"red_skill",fill:"crimson",onClick:"abrirTalento();",stroke:'black',strokeWidth:2});
...
function obtenerTalentos(svgt){
	var IdAdmin = Separ[1];
	$.post("/admins/json/ObtenerTalentos", {IdAdmin:IdAdmin},
			  function(jsonData) {
		var i=0;
	       for(i=0;i<jsonData.items.length;i++){
...
function ObtenerPorcentaje(){
	$.post("/admins/json/ObtenerPorcentaje", {},
			  function(jsonData) {
...
function version_explorer(){
	...
	if((version>0)&&(version<9)){
		document.location.href="/admins/mejora_la_web";  
	}else{
	}
...
function obtenerEmpleoInfo(id){
	 var idExp=id;
	 var IdUsuario2 = Separ[1];
	 $.post("/admins/json/ObtenerEmpleoInfo", {
	  idExp:idExp,IdUsuario2:IdUsuario2
	 },
	     function(jsonData) {
...
function obtenerEducacionInfo(id){
	 var idEdu=id;
	 var IdUsuario2 = Separ[1];
	 $.post("/admins/json/ObtenerEducacionInfo", {
	  idEdu:idEdu,IdUsuario2:IdUsuario2
	 },
	     function(jsonData) {
