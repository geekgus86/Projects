...
export function fetchDowntimesPerArea(from, to) {
  return async (dispatch, getState) => {
    try {
      const response = await api.get("/daily_production_reports/get_om", { val1: from, val2: to });
  ...
  return async (dispatch, getState) => {
    try {
      ...
      let shiftsPromises = _shifts.map(async (shift) => {
        return await api.get("/daily_production_reports/get_real_strokes_header", { val1: shift.startAt, val2: shift.endAt });
      })
      ...
      let detailsPromises = _shifts.map(async (shift) => {
        return await api.get("/daily_production_reports/get_shift_detail", { val1: shift.startAt, val2: shift.endAt });
      })
...
export function fetchRealStrokesHeader(from, to, id) {
  return async (dispatch, getState) => {
    try {
      const response = await api.get("/daily_production_reports/get_real_strokes_header", { val1: from, val2: to });
...
export function fetchDailyReportDetail(from, to, id) {
  return async (dispatch, getState) => {
    try {
      const response = await api.get("/daily_production_reports/get_shift_detail", { "val1": from, "val2": to, "id": id });
      if (id === "Turno1") {
...
export function fetchToolReports(from, to, tool, id, workOrder) {
  return async (dispatch, getState) => {
    try {
      const response = await api.get("/daily_production_reports/get_real_strokes_tool", { "val1": from, "val2": to, "tool": tool, "work_order_id": workOrder });
      const data = {
        tool: `${workOrder}-${id}`,
        data: response
      }
